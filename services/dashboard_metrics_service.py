...
DASHBOARD_METRICS_SCHEMA_VERSION = 3
...
def _merge_bucket(target: dict[str, Any], source: dict[str, Any]) -> None:
    ...
    target_outcomes = target.setdefault("by_endpoint_outcome", {})
    source_outcomes = source.get("by_endpoint_outcome") if isinstance(source.get("by_endpoint_outcome"), dict) else {}
    for endpoint, raw_counts in source_outcomes.items():
        if not isinstance(raw_counts, dict):
            continue
        counts = target_outcomes.setdefault(str(endpoint), {})
        for outcome, value in raw_counts.items():
            counts[str(outcome)] = int(counts.get(str(outcome), 0) or 0) + int(value or 0)
...
class DashboardMetricsService:
    ...
    def _apply_call(bucket: dict[str, Any], item: dict[str, Any]) -> None:
        ...
        if endpoint.startswith("/"):
            ...
            endpoint_outcomes = bucket.setdefault("by_endpoint_outcome", {}).setdefault(endpoint, {})
            endpoint_outcomes[outcome] = int(endpoint_outcomes.get(outcome, 0) or 0) + 1
    ...
    def _range_view(
        ...
        *,
        ...
        success_total = int(total_bucket.get("success", 0) or 0)
        failed_total = int(total_bucket.get("failed", 0) or 0)
        rate_limited_total = int(total_bucket.get("rate_limited", 0) or 0)
        final_failed_total = failed_total + rate_limited_total
        measured_total = success_total + final_failed_total
        switch_requests_total = int(total_bucket.get("switch_requests", 0) or 0)
        switch_recovered_total = int(total_bucket.get("switch_recovered", 0) or 0)
        totals = {
            ...
            "success_rate": round(success_total * 100 / measured_total, 2) if measured_total > 0 else None,
        }
        switching = {
            "requests": switch_requests_total,
            "count": int(total_bucket.get("switch_count", 0) or 0),
            "recovered": switch_recovered_total,
            "recovery_rate": (
                round(switch_recovered_total * 100 / switch_requests_total, 2)
                if switch_requests_total > 0
                else None
            ),
        }
        trend = {
        ...
        return {
            ...
            "text_review": totals["text_review"],
            "success_rate": totals["success_rate"],
            "by_endpoint": total_bucket.get("by_endpoint", {}),
            "by_endpoint_outcome": total_bucket.get("by_endpoint_outcome", {}),
