...
const DASHBOARD_VIEW_SCHEMA_VERSION = 1
...
function validateRange(value: unknown, path: string, expectedRange: DashboardTimeRangeKey) {
  ...
  ;['total', 'success', 'failed', 'rate_limited', 'text_review'].forEach((field) => {
    expectNumber(range[field], `${path}.${field}`, true)
  })
  expectNullableNumber(range.success_rate, `${path}.success_rate`)
  ;['by_endpoint', 'by_model', 'by_status', 'by_error_code'].forEach((field) => {
    expectCountRecord(range[field], `${path}.${field}`)
  })
  const endpointOutcomes = expectObject(range.by_endpoint_outcome, `${path}.by_endpoint_outcome`)
  Object.entries(endpointOutcomes).forEach(([endpoint, outcomes]) => {
    expectCountRecord(outcomes, `${path}.by_endpoint_outcome.${endpoint}`)
  })
  if (!Array.isArray(range.recent_failures)) contractError(`${path}.recent_failures`, 'array')
  range.recent_failures.forEach((failure, index) => expectObject(failure, `${path}.recent_failures[${index}]`))
  expectString(range.source, `${path}.source`)
  expectNumber(range.retention_days, `${path}.retention_days`, true)
}
...
function parseDashboardResponse(value: unknown): DashboardResponse {
  ...
  const selectedRange = expectRangeKey(meta.selected_range, 'response.meta.selected_range')
  ...
  validateRange(root.logs, 'response.logs', selectedRange)
