#!/usr/bin/env python3
"""Idempotently configure WARP as the default proxy and enable clearance support."""
...
def _looks_like_repository_default(runtime: Any) -> bool:
    ...
    candidate.pop("egress_mode", None)
    candidate.pop("proxy_url", None)
...
def _warp_default_proxy(existing_runtime: Any) -> str:
    configured = os.getenv("CHATGPT2API_DEFAULT_PROXY")
    if configured is not None:
        return configured.strip()
    legacy_configured = os.getenv("CHATGPT2API_PROXY_RUNTIME_PROXY_URL")
    if legacy_configured is not None:
        return legacy_configured.strip()
    if isinstance(existing_runtime, dict):
        legacy_mode = str(existing_runtime.get("egress_mode") or "").strip().lower()
        legacy_proxy = str(existing_runtime.get("proxy_url") or "").strip()
        if legacy_mode == "single_proxy" and legacy_proxy:
            return legacy_proxy
...
def main() -> int:
    ...
    changed = False
    force = _env_bool("CHATGPT2API_PROXY_RUNTIME_FORCE", False)
    default_proxy = _warp_default_proxy(existing)

    current_proxy = str(data.get("proxy") or "").strip()
    if default_proxy and (force or not current_proxy or current_proxy.lower() == "global"):
        data["proxy"] = default_proxy
        changed = True
        print("Configured WARP as the default proxy")
    ...
    if (
        ...
        changed = _deep_fill_missing(runtime, DEFAULT_PROXY_RUNTIME) or changed
    ...
    if runtime.pop("egress_mode", None) is not None:
        changed = True
    if runtime.pop("proxy_url", None) is not None:
        changed = True
    ...
    print(
        ...
        f"default_proxy={_mask_url(str(data.get('proxy') or ''))}, "
