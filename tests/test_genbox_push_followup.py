...
def test_unconfigured_push_makes_zero_network_requests(monkeypatch):
    ...
    with pytest.raises(push.GenBoxPushError) as exc:
        push.push_gallery_image("generated.png")
    assert exc.value.status_code == 409
    assert exc.value.code == "genbox_not_configured"
    assert calls == []
...
def test_non_image_content_is_rejected_before_network(monkeypatch):
    ...
    assert exc.value.status_code == 404
    assert exc.value.code == "genbox_source_not_found"
    assert calls == []
...
def test_push_timeout_returns_error_without_delete(monkeypatch):
    ...
    with pytest.raises(push.GenBoxPushError) as exc:
        push.push_gallery_image("generated.png")
    assert exc.value.code == "genbox_request_failed"
...
def test_auto_push_ignores_external_urls_without_spawning_network(monkeypatch):
    ...
    monkeypatch.setattr(
        push,
        "_submit_auto_push",
        lambda rel, *, metadata=None: submitted.append((rel, metadata)) or True,
    )
...
def test_auto_push_forwards_metadata_to_queue(monkeypatch):
    configure(monkeypatch)
    captured: list[tuple] = []
    monkeypatch.setattr(
        push,
        "_submit_auto_push",
        lambda rel, *, metadata=None: captured.append((rel, metadata)) or True,
    )
    push.auto_push_gallery_urls(
        ["http://app.invalid/images/registered.png"],
        metadata={"prompt": "a red square", "model": "gpt-image-2"},
    )
    assert captured == [("registered.png", {"prompt": "a red square", "model": "gpt-image-2"})]
